...
export const addBankDetails = async (userName, bankDetails) => {
  return await adminModel.updateOne({ userName }, { $push: { bankDetails } });
};


export const updateBankDetails = async (userName, bankDetailId, updatedFields) => {
  const updateQuery = {};
  for (const key in updatedFields) {
    updateQuery[`bankDetails.$.${key}`] = updatedFields[key];
  }

  return await adminModel.findOneAndUpdate(
    { userName, "bankDetails._id": bankDetailId },
    { $set: updateQuery },
    { new: true }
  );
};
...
export const removeBankDetail = async (userName, bankDetailId) => {
